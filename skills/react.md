...
function Risk({ project }: { project: Project }) {
  // domain logic in JSX layer
  const isAtRisk =
    project.milestones.filter((m) => m.status === 'blocked').length > 1;
  return <span>{isAtRisk ? 'At risk' : 'On track'}</span>;
}

// ✅ domain function, component just calls it
import { isAtRisk } from '@/domain/project';
function Risk({ project }: { project: Project }) {
  return <span>{isAtRisk(project) ? 'At risk' : 'On track'}</span>;
}
```
...
If unsure, ask: **does another screen need this?** If yes, lift. If no, keep it local.

---

## 5. Composition patterns to prefer

```tsx
// Slot composition — no prop explosion
<Card>
...
// Render-prop / children-as-fn for stateful primitives
<Disclosure>
  {({ open, toggle }) => (
...
// Context for cross-cutting state, not for prop avoidance
