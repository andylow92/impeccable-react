...
- [ ] Have you run `designGuard()` on the rendered tree and resolved every warning?
...
If a fail condition is hit, jump to section 4 (Rewrite instruction).
...
7. **Run `designGuard()`.** Resolve every warning before considering the
   rewrite complete.
